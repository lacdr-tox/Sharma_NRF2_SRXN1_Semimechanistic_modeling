...
Generic_function = function (Drug,Scenario,time_steps,Nrf2_plot,Obs_vs_Pred_plot,ktr_input) {
  ...
  xstart <- c(Nrf2_T1 = 0, Nrf2_T2 = 0,Srxn1 = 0)
  ...
  # Parmeters value
  ############################################
  parameters = data.frame(read.csv(paste0(parameters_dir,"/model_version_1.csv"),header = TRUE,stringsAsFactors = FALSE))
  ...
  parms <- unlist(c(data.frame(
    ...
    Km,
    hill,
    Ktr)))              #estimated from 24+24 scenarios (increment first dose + fixed second dose)
  ...
  SRXN1_mod1 <- function(t, x, parms,input1) {
    with(as.list(c(parms, x)), {
      ...
      dNrf2_T1 = Ktr * (NRF2_T - Nrf2_T1)
      dNrf2_T2 = Ktr * (Nrf2_T1 - Nrf2_T2)
      
      dSrxn1 = buildSrxn1Base + ((Vmax_buildSrxn1 * (Nrf2_T2**hill))/ (Km**hill + Nrf2_T2**hill)) - degradSrxn1 * Srxn1
      
      
      res <- c(dNrf2_T1,dNrf2_T2,dSrxn1)
  ...
  colnames(datafile_max) <- c("time" , "Nrf2_T1_max", "Nrf2_T2_max","Srxn1_max", "NRF2_max")
  colnames(datafile_mean) <- c("time2" ,"Nrf2_T1_mean", "Nrf2_T2_mean", "Srxn1_mean", "NRF2_mean")
  colnames(datafile_min) <- c("time3" ,"Nrf2_T1_min", "Nrf2_T2_min", "Srxn1_min", "NRF2_min")
...
Generic_function(Drug = Drug[2],      # 1 for Sul, 2 for Andro, 3 for Etha, & 4 for CDDO_me
                 ...
                 Obs_vs_Pred_plot =  "false",
                 ktr_input = 1) # assign true to plot observed vs predicted
