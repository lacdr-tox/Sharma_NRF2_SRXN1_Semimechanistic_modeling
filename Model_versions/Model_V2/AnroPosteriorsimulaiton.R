...
Setsim = paste0(drug[2], Modeling_dir,"setSim", ".out")
#chemical specific inputs and outputs
input = paste0(drug[2], Modeling_dir,"set.in", ".R")  # this file will contain the chemical inputs. 
ouput = paste0(drug[2], Modeling_dir,"set", ".out")  #check this output file name if it is the same or not
...
file$Chemical = rep(paste0("Andro"),each = nrow(file))
...
#save data
####
write.table(file,paste0(getwd(),"/", Modeling_dir,"/","Andro_posteriorsimulation.txt")) 

file = read.table(paste0(getwd(),"/", Modeling_dir,"/","Andro_posteriorsimulation.txt")) 
...
Generic_data_read(Drug = Drug[2],      # 1 for Sul, 2 for Andro, 3 for Etha, & 4 for CDDO_me
...
plot_function(Drug = Drug[2],      # 1 for Sul, 2 for Andro, 3 for Etha, & 4 for CDDO_me
              highdose = Toxicdose[2],
              Scenario = Scenario[4],  # 1 & 2 for repeated 8+24 & 24+24 respectively  , 2 & 4 for continous 32 and 48 respectively
              variable = "Srxn1")
