...
write(x, file=paste0(getwd(),"/", Modeling_dir, "/",input),append=TRUE)
  ...
  plot_function = function (Drug,highdose,Scenario,variable) {
    ...
    plot_final %>% 
      ...
      #geom_line(aes(x = Times, y = medianM)) +
      #geom_line(aes(x = Times, y = medianL)) +
      #geom_line(aes(x = Times, y = medianH)) +
      # geom_ribbon(aes(x = Times,ymin = LCLM, ymax = UCLM), fill = "blue", alpha = 0.5) +
      # geom_ribbon(aes(x = Times,ymin = LCLL, ymax = UCLL), fill = "green", alpha = 0.5) +
      # geom_ribbon(aes(x = Times,ymin = LCLH, ymax = UCLH), fill = "red", alpha = 0.5) +
      #geom_errorbar(aes(x = Times,ymin=Srxn1_observed-Srxn1_SD, ymax= Srxn1_observed +Srxn1_SD, size=0.5,width=.25)+
  ...
  geom_line(aes(x = Times, y = mean)) +
    geom_ribbon(aes(x = Times,ymin = LCL, ymax = UCL), fill = "green", alpha = 0.5)+
    geom_line(aes(x = Times, y = Nrf2_observed)) +
    geom_errorbar(aes(x = Times,ymin=Nrf2_observed-Nrf2_SD, ymax= Nrf2_observed +Nrf2_SD),  
                  size=0.5,width=.25)+