...
# added "Dose" term to check the dose dependency differential activation of Nrf2
...
Generic_function = function (Drug,Dose, Scenario,time_steps,Nrf2_plot,Obs_vs_Pred_plot) {
  ...
  dose_specific_Vmax =  ifelse(Dose == 'Dose1','Vmax_buildSrxn1.1.1.',
                               ifelse(Dose == 'Dose2',"Vmax_buildSrxn1.1.2.",
                                      ifelse(Dose == 'Dose3',"Vmax_buildSrxn1.1.3.",
                                             ifelse(Dose == "Dose4","Vmax_buildSrxn1.1.4.",
                                                    ifelse(Dose == "Dose5","Vmax_buildSrxn1.1.5.",
                                                           ifelse(Dose == "Dose6","Vmax_buildSrxn1.1.6.","NA"))))))
  
  parameters = data.frame(read.csv(paste0(parameters_dir,"/model_version_1.csv"),header = TRUE,stringsAsFactors = FALSE))
  
  dose_depen_parameters = data.frame(read.csv(paste0(parameters_dir,"/model_version1_dose_dep_Vmax.csv"),header = TRUE,stringsAsFactors = FALSE))
  ...
  Vmax_buildSrxn1 = (dose_depen_parameters %>% filter(X == dose_specific_Vmax) %>% select(mean))[[1]]
...
dose = c("Dose1", "Dose2", "Dose3", "Dose4", "Dose5", "Dose6")
...
Generic_function(Drug = Drug[1],      # 1 for Sul, 2 for Andro, 3 for Etha, & 4 for CDDO_me
                 Dose = dose[3],
                 Scenario = Scenario[3],  # 1 & 2 for repeated 8+24 & 24+24 respectively  , 2 & 4 for continous 32 and 48 respectively
                 time_steps,
