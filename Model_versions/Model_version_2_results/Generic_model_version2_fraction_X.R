...
# Model version 2 where there is  effect of fraction x
# Nrf2 does require any co-activator to activate the Srxn1?
...
output_dir ="C:/Users/alexe/OneDrive - Universiteit Leiden/mcsim_files/Model_versions/Model_version_2_results"
...
Generic_function = function (Drug,Scenario,time_steps,data_save,Nrf2_plot,Fraction_X_plot,Obs_vs_Pred_plot) {
  ...
  chem_specific_fraction_x =  ifelse(Drug == 'Sulforaphane','fraction_x.1.1.',
                            ifelse(Drug == 'Andrographolide',"fraction_x.1.2.",
                                   ifelse(Drug == 'Ethacrynic_Acid',"fraction_x.1.3.",
                                          ifelse(Drug == "CDDO_me","fraction_x.1.4.", "NA"))))
  ...
  # Parmeters value
  ############################################
  parameters = data.frame(read.csv(paste0(parameters_dir,"/model_version_2_fractionx.csv"),header = TRUE,stringsAsFactors = FALSE))
  ...
  Vmax_buildSrxn1 = (parameters %>% filter(X == "Vmax_buildSrxn1.1.") %>% select(mean))[[1]]
  Km = (parameters %>% filter(X == "Km.1.") %>% select(mean))[[1]]
  hill = (parameters %>% filter(X == "hill.1.") %>% select(mean))[[1]]
  Fraction_X = (parameters %>% filter(X == chem_specific_fraction_x) %>% select(mean))[[1]]
  Km_act = (parameters %>% filter(X == "Kact.1.") %>% select(mean))[[1]]
  hill_act = (parameters %>% filter(X == "hill_act.1.") %>% select(mean))[[1]]
  ...
  SRXN1_mod1 <- function(t, x, parms,input1) {
    with(as.list(c(parms, x)), {
      ...
      Nrf2_modified = Fraction_X*NRF2_T
      Nrf2_unmodified = NRF2_T*(1-Fraction_X)
      
      
      dSrxn1 = buildSrxn1Base + (Vmax_buildSrxn1 * (Nrf2_unmodified**hill) *(Nrf2_modified**hill_act))/
        ((Km**hill + Nrf2_unmodified**hill) *(Km_act**hill_act + Nrf2_modified**hill_act)) - degradSrxn1 * Srxn1
...
Finalplotdata %>%  write.table(.,paste0(output_dir,"/",Drug,"_",Scenario,".txt"))  
}
# End of Function
#################################################################################################
...
Generic_function(Drug = Drug[4],      # 1 for Sul, 2 for Andro, 3 for Etha, & 4 for CDDO_me
