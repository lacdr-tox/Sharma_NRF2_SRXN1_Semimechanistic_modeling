...
R_square <- cor(Final2$SRXN1_observed, Final2$Srxn1_mean)^2
lim = max(Final2$Srxn1_mean)*1.2
version = c("V.1", "V.2")
dose =ifelse(i %in%c(1,3,5,7,9,11,13,15),"low","high")
name = c("A.", "B.","C.", "D.", "E.", "F.", "G.", "H.", "I.")
version = ifelse(i >= 1 & i<9, "V.1", "V.2")
plot(Final2$SRXN1_observed, Final2$Srxn1_mean,
       xlim = c(0.1, lim), ylim = c(0.1, lim),
       xlab = "Observed", ylab = "Predicted",
       main = paste(Final2$Treament[1],version,dose, "(R2 =", round(R_square, digits = 3),")"),cex.main = 1)
  abline(0,1)
